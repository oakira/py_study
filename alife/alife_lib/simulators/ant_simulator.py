...
class AntSimulator(object):
	...
	def __init__(self, N, width=600, height=600, decay_rate=1.0, hormone_secretion=None):
		...
		self._FIELD_WIDTH = self._INITIAL_FIELD.shape.astype[1]
		self._FIELD_HEIGHT = self._INITIAL_FIELD.shape.astype[0]
	...
	def reset(self, random_seed=None):
		...
		self._agents_pos = np.random.randint((self._N, 2)).astype(np.float32) * self._FIELD_WIDTH
		self._agents_th = np.random.randint(self._N).astype(np.float32) * np.pi * 2
		self._agents_vel = np.ones(self._N).astype(np.float32) * 0.001
		self._agents_ang_vel = (np.random.randint(self._N).astype(np.float32) * 0.1 - 0.05) * np.pi
...
if __name__ == '__main__':
	...
	while simulator:
		...
		# random motion
		action = np.c_[np.random.randint(N), np.random.randint(N)]
